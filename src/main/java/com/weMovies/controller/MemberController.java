...
public class MemberController {
     ...
     * 
     * @RequestMapping("login_check.do") public String login(LoginDTO ldto,
     * HttpSession session, Model model) throws Exception { String name =
     * service.login(ldto); if (name != null) { session.setAttribute("mid",
     * ldto.getMid()); // session.setAttribute("name", ldto.getName());
     * model.addAttribute("msg", "로그인에 성공하셨습니다."); model.addAttribute("url", ""); }
     * else { model.addAttribute("msg", "로그인에 실패하셨습니다. 다시 시도해주세요.");
     * model.addAttribute("url", "/login.do"); } return "alert/message"; }
     */
